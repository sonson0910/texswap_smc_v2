    ...
    switch (choice.trim()) {
      case '1':
        try {
          ...
          console.log(`\n  ✗ Deploy failed: ${msg}`);
          // VERBOSE DEBUG: Log full error details
          if (e instanceof Error) {
            console.error(`\n  ═══ FULL ERROR DETAILS ═══`);
            console.error(`  Message: ${e.message}`);
            console.error(`  Stack: ${e.stack}`);
            logger.error({ err: e, stack: e.stack, cause: e.cause }, `Deploy error: ${e.message}`);
          }
      ...
      case '2':
        try {
          const contractAddress = await rli.question('Enter the contract address (hex): ');
          const contract = await api.withStatus('Joining existing contract', () =>
            api.joinContract(providers, contractAddress.trim()),
          );
          console.log(`  Contract joined at: ${contract.deployTxData.public.contractAddress}\n`);
          return contract;
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Failed to join contract: ${msg}\n`);
          logger.error({ err: e }, `Join contract error: ${msg}`);
        }
        break;
      case '3':
    ...
    switch (choice.trim()) {
      case '1':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Mint X tokens failed: ${msg}`);
      ...
      case '2':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Mint Y tokens failed: ${msg}`);
      ...
      case '3':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Init liquidity failed: ${msg}`);
      ...
      case '4':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Add liquidity failed: ${msg}`);
      ...
      case '5':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Remove liquidity failed: ${msg}`);
      ...
      case '6':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Swap X to Y failed: ${msg}`);
      ...
      case '7':
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          logger.error(`Swap Y to X failed: ${msg}`);
      ...
      case '8':
        try {
          ...
          console.log('   Mint functions create tokens from nothing (faucet).');
          console.log('   Liquidity & swap operations consume real tokens from your wallet.');
          console.log('   Pool status is tracked client-side (ledger reading requires indexer).');
